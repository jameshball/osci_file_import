#include "osci_WavParser.h"


WavParser::WavParser(SampleRateProvider provider) : sampleRateProvider(std::move(provider)) {}
...
bool WavParser::parse(std::unique_ptr<juce::InputStream> stream) {
    ...
    const double processorRate = getProcessorSampleRate();
...
double WavParser::getProcessorSampleRate() const {
    return sampleRateProvider != nullptr ? sampleRateProvider() : 0.0;
}
...
void WavParser::processBlock(juce::AudioBuffer<float> &buffer) {
    ...
    if (followProcessorSampleRate.load()) {
        const int desired = (int) getProcessorSampleRate();
