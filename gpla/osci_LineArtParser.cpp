...
std::vector<std::vector<osci::Line>> LineArtParser::fallbackFrames() {
    return {{
        osci::Line(-0.5, -0.5, 0.5, 0.5),
        osci::Line(-0.5, 0.5, 0.5, -0.5)
    }};
}
...
std::vector<std::vector<osci::Line>> LineArtParser::parseJsonFrames(juce::String jsonStr) {
    ...
    if (numFrames == 0) return epicFail();
    ...
    if (!hasValidFrames) return epicFail();
